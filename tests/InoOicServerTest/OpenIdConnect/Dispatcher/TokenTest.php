...
class TokenTest extends DispatcherTestCase
{
    ...
    {
        $response = $this->getMockBuilder('\InoOicServer\OpenIdConnect\Response\Token')
            ->disableOriginalConstructor()
            ->setMethods(array())
            ->getMock();
    ...
    {
        $registry = $this->getMockBuilder('\InoOicServer\Client\Registry\Registry')
            ->disableOriginalConstructor()
            ->getMock();
    ...
    protected function _getClientAuthenticationManagerStub($authSuccess = false, $failureReason = 'auth failure reason')
    {
        ...
        $clientAuthenticationManager = $this->getMock('InoOicServer\Client\Authentication\Manager');
