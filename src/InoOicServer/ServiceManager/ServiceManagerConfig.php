    ...
    public function getFactories()
    {
        return array(
            ...
            'Zend\Db\Adapter\Adapter' => function ($sm)
            {
                ...
                if (! isset($config['oic_server']['db']['adapter']) || ! is_array($config['oic_server']['db']['adapter'])) {
                    throw new Exception\MissingConfigException('oic_server/db/adapter');
                }
                
                return new \Zend\Db\Adapter\Adapter($config['oic_server']['db']['adapter']);
            },
            ...
            'InoOicServer\Oic\User\Authentication\Manager' => function ($sm)
            {
                ...
                return new User\Authentication\Manager($options, $urlHelper);
            },
            ...
            'InoOicServer\Oic\Authorize\Http\HttpService' => function ($sm)
            {
                $config = $sm->get('Config');
                $options = array();
                if (isset($config['oic_server']['authorize_http_service']) && is_array($config['oic_server']['authorize_http_service'])) {
                    $options = $config['oic_server']['authorize_http_service'];
                }
                
                $userAuthenticationManager = $sm->get('InoOicServer\Oic\User\Authentication\Manager');
                $sessionContainer = $sm->get('Zend\Session\Container');
                
                $httpService = new HttpService($options, $userAuthenticationManager, $sessionContainer);
            ...
            {
                $mapper = new AuthSession\Mapper\DbMapper($sm->get('Zend\Db\Adapter\Adapter'));
            ...
            {
                $mapper = new Session\Mapper\DbMapper($sm->get('Zend\Db\Adapter\Adapter'));
            ...
            {
                $mapper = new AuthCode\Mapper\DbMapper($sm->get('Zend\Db\Adapter\Adapter'));
